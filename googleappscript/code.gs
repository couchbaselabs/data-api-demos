const findEndpoint = 'https://dapi.couchbase.live/v1/api/scopes/_default/collections/_default/docs?limit=100';
...
function getResultHeaders() {
 ...
 let connectString = getAPIKey('APICONNECTSTRING', 'couchbase connect string or hostname')
 if (connectString == null || connectString == '') {
   connectString = 'couchbase://localhost';
 }
 if (!connectString.includes('couchbase://') && !connectString.includes('couchbases://')) {
     if (connectString.includes('localhost')) {
       connectString = 'couchbase://' + connectString;
     } else {
      connectString = 'couchbases://' + connectString;
     }
 }

 let apiAccesskey = getAPIKey('APIACCESSKEY', 'access key or username')
 if (apiAccesskey == null || apiAccesskey == '') {
   apiAccesskey = 'Administrator'
 }
 ...
 let dbName = getAPIKey('APIBUCKETNAME', 'db or bucket name')
 ...
 const options = {
  ...
  headers: { 
    "X-CB-DAPI-CONNECT-STRING": connectString, 
    "X-CB-DAPI-ACCESS-KEY": apiAccesskey, 
    "X-CB-DAPI-SECRET-KEY": apiSecretkey,
    "X-CB-DAPI-NAME": dbName 
    }
 };
